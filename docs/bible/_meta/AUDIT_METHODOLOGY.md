...
**Status:** LOCKED v3-patched-i (2026-05-19)
...
- v3-patched-i (2026-05-19): REPAIR-5 dispatch — § 4.40 (rolling-window aggregate AS-OF discipline; compute_speed_figures.py Step A case study) + § 4.41 (multi-phase retrain dependency orchestration; 39-model retrain wave 4-phase pattern). Pattern: substrate-fix dispatch for the last code-class leakage surface (rolling-window par/norm in training pipeline) + substrate-extension of REPAIR-4 aggregate-history pattern to trainer_stats. Migration 013_trainer_stats_history.sql applied to production. Retrain orchestration + cutover SQL authored substrate-pragmatic for 39-model retrain wave (substrate-execution deferred to Tony trigger; substrate-wall-clock substantial). Tier 3 surgical patch under REPAIR-5 dispatch ceremony cap; same Q5 ratification pattern as v3-patched-b through v3-patched-h.
...
### 4.40 — Rolling-window aggregate AS-OF discipline at training-data preprocessing (REPAIR-5 Step A case study)

**Abstract rule**: When training-data preprocessing computes aggregate statistics (par times, normalization constants, percentile thresholds) over the full historical corpus then applies them uniformly to every row, the substrate-result is leakage of FUTURE aggregate state into PAST rows. A 2022 row computed against par times that include 2024-2026 data substrate-sees a normalized speed figure that depends on data that did not yet exist at the row's substrate-actual race date. The substrate-correct pattern is rolling-window or per-period (year/quarter/month) aggregation with strict-less-than AS-OF semantics: row from period P uses ONLY aggregate statistics computed from data in periods strictly < P.

**Worked example**: Equine Equalizer compute_speed_figures.py pre-REPAIR-5 computed par times by `GROUP BY (track, distance, surface)` over the full past_performances corpus, producing one par_time per bucket. Each row's speed_rating_raw was then computed using `par_map[(track, distance, surface)]` — substrate-identical for every row regardless of race_date. Substrate-effect: 2022 horse normalized against par from a corpus including 2024-2026 races; training models on those figures saw substrate-future aggregate state. Same leakage class in Step 6 Beyer normalization: median + std computed across ALL adjusted figures, applied uniformly.

REPAIR-5 fix (commit 17e9486):
- Step 2: `GROUP BY ..., EXTRACT(YEAR FROM race_date)`; par_map keyed by 4-tuple including year
- bucket_years index per (track, distance, surface); lookup_par(tc, d, s, target_year) via bisect_left strict-less-than (returns par from most recent year < target_year, or None if no prior data)
- Step 3 callsite: row_year = race_date.year; par = lookup_par(...)
- Step 6 Beyer norm parallel: per_year_stats query GROUP BY year on computed_speed_figure; norm_map + norm_years_sorted index; lookup_norm() strict-less-than; per-year UPDATE filtered by EXTRACT(YEAR FROM race_date)

**Cross-references**: § 4.38 (AS-OF discipline at row-level — per-row historical pps); § 4.39 (aggregate-without-history at inference-time — angle_stats / trainer_stats pattern). § 4.40 is the THIRD class of AS-OF discipline: pre-aggregated training-input statistics that are baked into the inputs themselves (not queried at runtime).

**Audit-CC prophylactic check template**: At training-data preprocessing code review time: (1) substrate-grep every aggregate computation (PERCENTILE_CONT, AVG over groups, STDDEV, MIN/MAX over date-spanning ranges); (2) verify the aggregate is either (a) per-period bucketed AND applied with strict-less-than period lookup, OR (b) computed per-row via window function with `RANGE BETWEEN ... PRECEDING AND <strict> PRECEDING` exclusive-of-target-row, OR (c) explicitly documented as "same-day substrate-correct" (daily variants pattern); (3) for any cross-period aggregate applied to mixed-period rows, flag as rolling-window-leakage class; (4) substrate-pragmatic substrate-fix discipline: per-year bucketing with bisect lookup is acceptable approximation when row-level rolling window is computationally substrate-prohibitive.

---

### 4.41 — Multi-phase retrain dependency orchestration discipline (REPAIR-5 Step C case study)

**Abstract rule**: When the production model corpus has internal dependency structure (L2 models consume L1 outputs; ensembles consume specialists; specialists may share preprocessing artifacts), a retrain wave MUST execute phases in strict dependency order with substrate-completion gates between phases. Parallel-launch-within-phase substrate-respects dependency closure; sequential-between-phases prevents L2 models training on stale L1 artifacts. Cutover (flipping is_active) MUST be a single transaction over ALL retrained models — partial cutover leaves production substrate-incoherent (mix of clean L1 + stale L2 or vice versa).

**Worked example**: Equine Equalizer REPAIR-5 retrain wave for 39 contaminated models:
- Phase 1 (parallel, 30 tasks): L1 specialists — pl/win_prob/ranker variants. No cross-deps within phase.
- Phase 2 (1 task): Hybrid C ensemble — depends on Phase 1 L1 artifacts.
- Phase 3 (4 tasks): logistic ensemble + trajectory LSTM + longshot RF + wr (substrate-independent of Hybrid C but logically L2).
- Phase 4 (2 tasks): ranker_core + ranker_full + win_prob_full singletons.

Substrate-discipline:
- Per-phase: launch via ECS Fargate run-task; poll describe-tasks until STOPPED; verify exit code 0 + new model_versions row inserted + S3 artifact uploaded.
- HALT ENTIRE WAVE on first task failure — do NOT proceed to subsequent phases or cutover with partial retrain.
- Tag new rows in notes field with `clean_post_repair5_<YYYYMMDD>` for substrate-cutover-identification.
- Single-transaction cutover (BEGIN/COMMIT) flipping is_active across all 39 contaminated → clean rows; partial unique idx `idx_active_model_per_type WHERE is_active=true` substrate-validates exactly-one-active-per-type post-COMMIT.

**Cross-references**: § 4.34 (forensic-gate activation discipline — same single-transaction substrate-pattern for activation flips); § 4.32 (handoff-authoring-without-substrate-verification — dependency graph claims MUST be substrate-verified against training-script input loaders before phase assignment, not inferred from naming conventions); § 4.16 (lock-CC metadata bundle — retrain wave produces operational metadata recorded in model_versions.notes parallel to bible metadata bundles).

**Audit-CC prophylactic check template**: At retrain orchestration script review time: (1) substrate-discover dependency graph by inspecting each training script's input loaders (which S3 paths / DB tables / prior model artifacts it reads); (2) substrate-verify phase assignment respects discovered dependencies (no Phase N task reads artifact produced in Phase M where M > N); (3) substrate-verify HALT discipline: any single task failure prevents subsequent phase launches AND cutover; (4) substrate-verify cutover SQL is single transaction (BEGIN/COMMIT) with read-back verification before COMMIT; (5) substrate-verify cutover SQL includes verbatim PK list of contaminated rows (not pattern-based selection which could miss edge cases); (6) substrate-verify retrain EventBridge rules remain DISABLED until cutover completes (otherwise concurrent cron-fired retrains contaminate the wave with mixed-state).
