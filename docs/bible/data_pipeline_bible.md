...
Status: LOCKED v1-patched-f (2026-05-12) — Phase A E4 Step 3 Path A reconciliation closure: § 4.4 cross-axis source inventory post-reconciliation patch + § 4.2.6 Phase 5 backlog routing patch; supersedes LOCKED v1-patched-e (2026-05-12 D6)
...
- v1-patched-f (2026-05-12): Phase A E4 Step 3 Path A reconciliation closure dispatch — § 4.4 cross-axis source inventory post-reconciliation patch + § 4.2.6 Phase 5 backlog routing patch. **Override disclosure (Q5 ratification preserved):** Tier 2 surgical F.4 patch under Phase A entry directive ceremony cap. UC § 7.2 step 4 per-bible patch-CC convention explicitly overridden by ceremony cap. **2 patches applied at SP-A5:** BP-A5.3 § 4.4 cross-axis source inventory post-reconciliation status (Source 4 + Source 5 OUT-OF-BAND classification preserved per E1 Path 2 RE-SCOPED V2 design-state-since-deployment verdict; Source 5 cron-mode-failure-by-design documented as known operational state); BP-A5.5 § 4.2.6 Phase 5 backlog routing patch (deferred-disposition-encoding for operational gaps removed per Tony banking ratification this session; operational gaps route to current-cycle resolution per Directive 2 default-cadence). Cross-bible cross-references: § 4.4 cross-references `architecture_overview:3.12` Appendix C (NEW OUT-OF-BAND classification at v3-patched-f); § 4.2.6 cross-references `AUDIT_METHODOLOGY:4.32` (handoff-authoring-without-substrate-verification refinement) for the deferred-disposition-encoding methodology lesson. Cross-bible cross-reference freeze NOT re-engaged for SP-A5 (Tier 2 ceremony cap pattern).
- v1-patched-e (2026-05-12): Phase A D6 bundled bible patches dispatch. **Override disclosure (Q5 ratification):** D6 surgical patches per F.4 pattern under Tier 2 ceremony cap per Phase A entry directive. UC § 7.2 step 4 per-bible patch-CC convention explicitly overridden by ceremony cap. Rationale: D6 documents Phase A operational findings (A.5 / A.5.1 / A.5.2 / A.5.3 / A.5-ext / A.6.a–f / D2 / D3 / D4 dispatch sequence) into bibles per Phase A re-dispatch venue (R14.2 Option A scope); not a running cross-bible-cross-reference-freeze UC cycle. **5 patches applied** per Phase A handoff `docs/operations/PHASE_A_HANDOFF_2026-05-12.md` §§ 1.1, 2.3, 2.5, 2.6, 2.15: DP-1 NEW § 4.4 Operational source inventory (5-source cross-axis view absorbing DP-5 workouts source architecture per Phase A handoff § 2.6); DP-2 § 4.2.4 NYRA refresh (Q4 closure of R14.2 Option A deferred-disposition reservation per Phase A handoff § 1.1 + § 1.5 + § 1.6); DP-3 NEW § 4.5 AWS API validation discipline (per Phase A handoff § 2.3 A.5-ext step-ordering banking; cross-references AUDIT_METHODOLOGY § 4.30 case-study + § 4.31 producer-attribution refinement); DP-4 NEW § 4.6 predict_race internal filter + A.5.3 fix (per Phase A handoff § 2.5 A.5.3 surgical fix banking commit e1d6d4a); DP-6 NEW § 4.7 scratched-after-prediction surplus pattern (per Phase A handoff § 2.15 D2-γ classification). Cross-bible cross-reference freeze status: NOT re-engaged for D6 (Tier 2 ceremony cap pattern); per-bible audit-CC tier not invoked at D6 scope.
...
#### 4.2.4 NYRA workouts (refreshed at v1-patched-e per Q4 closure of R14.2 Option A deferred-disposition reservation)
...
**Current reliability state (refreshed 2026-05-12 at v1-patched-e per Phase A D6 dispatch).** Empirically verified via four-element substrate chain:

  1. **Lambda runtime State** per `architecture_overview:3.1`: `equine-nyra-workouts` Active (memory 512 MB, timeout 300s; LastModified 2026-04-27T22:11:00Z UTC).
  2. **EventBridge cron schedule (retimed)** per `architecture_overview:3.6` line 154 + § 4.1.4 Finding 1 closure block: cron `cron(0 16 * * ? *)` = 16:00 UTC daily (retimed at OCRC Fix 3 2026-05-09 from prior `cron(0 10 * * ? *)` = 10:00 UTC per `docs/operations/CRON_PAYLOAD_AUDIT_2026-05-09.md` + OCRC close-out § 1.2 item 5). The pre-retiming 10:00 UTC cron executed BEFORE NYRA publishes daily workout data on its endpoint, producing 7+ consecutive days of zero-result scrapes pre-OCRC.
  3. **Post-OCRC Fix 3 fix-time evidence.** Manual invocation 2026-05-09T16:12:33Z UTC captured **99 workouts** (SAR=50 + BEL=49 + AQU=0) vs pre-fix 7-day zero-baseline. Post-fix substrate stability verified at V25 SP-resume 2026-05-11T14:31:09Z UTC.
  4. **DLQ wiring (post A.5-ext)** per Phase A handoff § 1.2 + § 1.5: A.5-ext dispatch 2026-05-12T01:09:13-04:00 applied event-invoke-config to `equine-nyra-workouts` with OnFailure → `arn:aws:sqs:us-east-1:584812014683:equine-async-failure-dlq`, MaximumRetryAttempts=2, MaximumEventAgeInSeconds=3600. AsyncDLQSend inline policy applied to `equine-nyra-workouts-role` granting `sqs:SendMessage` on DLQ ARN. F-D4-1-α gap closed; 6-Lambda async-drop-affected coverage class complete per `architecture_overview:3.10` extension.

Current reliability: post-Fix-3 substrate-stable and post-A.5-ext DLQ-wired. NYRA workouts table receives fresh rows daily on retimed 16:00 UTC schedule. Confirmed-operationally-functional per Phase A handoff § 1.1 Source 3 inventory.

**Failure manifestation.** None documented at v1-patched-e lock per the four-element substrate verification above. Pre-OCRC failure mode (cron-vs-publication-window misalignment producing 7+ days zero-result scrapes) resolved at OCRC Fix 3. Future failure modes captured via DLQ depth alarm `equine-async-dlq-messages-present` (pages ≤ 5 min on async-drop) + `equine-nyra-workouts-{errors,throttles,invocations-absence,daily-cron-absence}` per `architecture_overview:3.10`.

**Current acquisition mode.** Autonomous.

**Honest disposition.** Autonomous (matches current mode). § 4.2.4 deferred-disposition reservation per R14.2 Option A scope exclusion **CLOSED** at v1-patched-e per Phase A handoff substrate.

**Manual recovery tool.** Per Phase A handoff § 1.5 A.6.b deliverable: `scripts/manual_nyra_workouts.py` for ad-hoc NYRA workouts backfill. CLI args: `--start-date YYYY-MM-DD --end-date YYYY-MM-DD [--tracks SAR,BEL,AQU] [--execute]`; default behavior is dry-run, `--execute` applies writes. See `docs/operations/DAILY_INGESTION_RUNBOOK.md § 3.1` for usage pattern + smoke-test SQL.

**Cross-reference.** Per-flow detail at § 4.1.4 (Finding 1 closure block already documented at v1-patched-d). Operational source inventory cross-axis view at § 4.4 (D6 v1-patched-e patch).
...
**Phase 5 backlog routing patch (BP-A5.5 v1-patched-f patch).** Per Tony banking ratification this session (parallel-dispatch Decision 4 + Directive 2 reinforcement): **the deferred-disposition-encoding pattern for operational gaps is removed.** Operational gaps route to current-cycle resolution per Directive 2 default-cadence rather than accumulating as `PHASE_5_BACKLOG.md` entries. The probe-script cohort disposition remains a strategic decision (kill / paid-replacement / scheduled-manual) requiring Tony adjudication, but the "pending Phase 5 entry" framing is no longer the operational pattern. Operational gaps that surface during current-cycle work are resolved during current-cycle work; only strategic-class decisions (multi-option scope; cost-vs-benefit deliberation) escalate to Tony adjudication without Phase 5 deferral encoding.

This change applies to the broader "Phase 5 backlog" framing pattern across this bible (and other bibles). Prior § 4.2.x "Honest disposition" subsections use Phase 5 backlog framing for operational gap deferrals; per BP-A5.5 routing patch, such framings should be re-evaluated at next surface for current-cycle-resolution rather than Phase 5 deferral. Banking observation: this is the deferred-disposition-encoding methodology refinement codified at `AUDIT_METHODOLOGY:4.32` § 4.32 Case Study patterns; recursive application discipline applies to bible substrate.
...
### 4.4 Operational source inventory (cross-axis view; D6 v1-patched-e patch per Phase A handoff § 1.1 + § 2.6; REVISED at v1-patched-f per Phase A E4 Step 3 Path A reconciliation closure)

**Post-reconciliation status update (BP-A5.3 v1-patched-f patch).** Phase A E4 Step 3 Phase 2 Path A reconciliation closure transitioned 50 CLI-only resources within `EquineComputeStack` scope to CFN-managed status. **Within EquineComputeStack: 0 CLI-only resources remain.** Outside `EquineComputeStack` scope: sibling-repo Equibase scraper at `/home/strakajagr/equibase_scraper/` retains OUT-OF-BAND classification (Source 4 Equibase workouts producer + Source 5 Equibase charts producer; per `architecture_overview:3.12` Appendix C NEW classification at v3-patched-f). The OUT-OF-BAND classification is distinct from ORPHAN: sibling-repo is operator-controlled infrastructure on Tony's local machine, not a candidate for AWS-side CFN reconciliation. Documented as design-state-since-deployment per E1 Path 2 RE-SCOPED V2 substrate-investigation verdict.

**Source 5 known operational state (per E1 Path 2 RE-SCOPED V2 substrate-investigation closure)**: scraper has been in design-state red since deployment 2026-04-27 evening per README explicit "Charts under cron — known limitation" section. Cron-mode `--unattended` failure mode is intentional design-state, NOT regression. 15/15 captured cron runs failed identically since 2026-04-28. Zero cron-time-pattern S3 chart-PDF uploads ever existed. Operational pattern: daily manual operator workaround required for chart freshness. Source 5 alarm scaffolding behavior: SNS alerts to `arn:aws:sns:us-east-1:584812014683:equine-equalizer-alerts` fire daily on `final_exit_code=1`; this is design-correct alarm behavior reporting design-state failure mode. Operator-future-self surface: this is NOT a regression to investigate; codification of daily-manual-operation pattern in D5 runbook pending; Branch β WSLg viability investigation deferred to separate operational venue.

Per-flow detail at § 4.1 enumerates the 9 cron-triggered flows in flow-by-flow narrative form. § 4.2 enumerates the 6 data-acquisition sources in per-source reliability form. This section provides a cross-axis source-by-source operational inventory for the 5 operational sources that produce daily data to the EE pipeline. Source 4 + Source 5 are out-of-band producers (sibling repo on Tony's local machine) that this bible documents for completeness; their substrate is not in `/home/strakajagr/projects/`.

| # | Source | Producer | Schedule (UTC) | Tables / S3 written | DLQ | Per-flow detail |
|---|---|---|---|---|---|---|
| 1 | HRN entries | `equine-ingestion` Lambda (default action) | `cron(0 11 * * ? *)` = 11:00 | entries, races, horses, trainers, jockeys, tracks, past_performances | ✅ (Phase A-prime; 2-Lambda DLQ wiring) | § 4.1.1 |
| 2 | HRN results | `equine-ingestion` Lambda (action=fetch_results via InputTransformer) | `cron(30 1 * * ? *)` = 01:30 | results | ✅ (inherited from Lambda) | § 4.1.2 |
| 3 | NYRA workouts | `equine-nyra-workouts` Lambda | `cron(0 16 * * ? *)` = 16:00 (retimed OCRC Fix 3 2026-05-09) | workouts (via S3 → `load_workouts_from_s3`) | ✅ (A.5-ext 2026-05-12) | § 4.1.4 |
| 4 | Equibase workouts | `/home/strakajagr/equibase_scraper/run_daily_refresh.sh` (local cron, sibling repo) | `0 3 * * *` (03:00 EDT = 07:00 UTC) | workouts (via S3 → `load_workouts_from_s3`) | N/A (out-of-band) | (out-of-band) |
| 5 | Equibase charts | `/home/strakajagr/equibase_scraper/download_charts.py` (same cron entry as Source 4) | (same cron entry as Source 4) | PDF charts to `s3://equine-raw-data/charts/` | N/A (out-of-band; FAILING DAILY exit=1) | (out-of-band; in-bible chart parser entry-point at § 4.1.3 invoked via `parse_charts` admin action on Source 1's Lambda) |

**Source 2 architecture clarification (per Phase A handoff § 1.1 + AUDIT_METHODOLOGY § 4.29 D4 conflation banking).** HRN-results-scraping is `equine-ingestion` Lambda's `fetch_results` action, NOT the `equine-results` matcher Lambda. The `equine-results` Lambda is a separate matcher / reconciliation Lambda (cron `cron(0 4 * * ? *)` = 04:00 UTC via EventBridge rule `equine-results-daily` per `architecture_overview:3.6` line 156); it updates `wr_predictions.actual_finish` against arriving results, NOT data ingestion. The two Lambdas were conflated in pre-D4 dispatch text; CC verification at D4 surfaced the distinction and AUDIT_METHODOLOGY § 4.29 banks the methodology lesson.

**Source 3 + Source 4 workouts source architecture (per Phase A handoff § 2.6).** Two daily producers feed the `workouts` table via the shared `s3://equine-raw-data/workout-loads/` S3 prefix → `equine-ingestion` Lambda's `load_workouts_from_s3` action:

- **Producer 1 (Source 3): NYRA Lambda cron @ 16:00 UTC.** Lambda `equine-nyra-workouts`. Coverage: SAR + BEL + AQU only. S3 path: `s3://equine-raw-data/workout-loads/{YYYYMMDD}_nyra_{YYYYMMDD_HHMMSS}.json` (NYRA-infix marker for producer attribution at S3-listing layer). Architectural: ORPHAN (CLI-deployed; not in CDK source per `architecture_overview:3.1`).
- **Producer 2 (Source 4): Equibase sibling-repo cron @ 03:00 EDT / 07:00 UTC.** Producer `/home/strakajagr/equibase_scraper/collect_daily_workouts.py` invoked by `run_daily_refresh.sh`. Local crontab line: `0 3 * * * SNS_TOPIC_ARN=arn:aws:sns:us-east-1:584812014683:equine-equalizer-alerts AWS_DEFAULT_REGION=us-east-1 PATH=...:/home/strakajagr/equibase_scraper/venv/bin /home/strakajagr/equibase_scraper/run_daily_refresh.sh >> /home/strakajagr/equibase_scraper/logs/cron.log 2>&1`. Coverage: 8+ tracks per per-horse iteration (CD / GP / KEE / MTH / SA / OP plus NYRA-overlap SAR / BEL / AQU). S3 path: `s3://equine-raw-data/workout-loads/{YYYYMMDD}_{HHMMSS}.json` (NO `_nyra_` infix). Architectural: OUT-OF-BAND (sibling repo on Tony's local machine; not in `/home/strakajagr/projects/`).

**Convergence.** Both producers' S3 outputs are loaded into the `workouts` DB table via `equine-ingestion` Lambda's `load_workouts_from_s3` admin action. The shared S3 prefix + shared loader path mean producer attribution at the DB-row level is NOT directly available without substrate archaeology (S3 listing filtered to NYRA-infix vs non-NYRA-infix); workouts source-column schema-evolution candidate documented at `database_schema_bible:4.1.8` for future single-SQL-query producer attribution.

**Pre-A.6.f framing correction (banked at AUDIT_METHODOLOGY § 4.27 + § 4.31).** Pre-A.6.f session memory included framing of "Tony's local HRN scrape ~4000 rows/day daily operational backbone". A.6.f verification (2026-05-12) surfaced that:
- No local HRN scrape script exists at `/home/strakajagr/projects/` or `/home/strakajagr/equibase_scraper/`.
- The daily producer is Equibase sibling-repo, NOT HRN.
- The volume claim was misread: Lambda `inserted=N` log value is records-processed-from-payload (UPSERT-collapsed), NOT net-new-DB-inserts; net-new rows yield ~12–22% of file volume per UPSERT collapse.

Pre-A.6.f framing was operational-signal-inference inherited from session memory and propagated to A.6.d dispatch text without substrate verification. A.6.f investigation closed the inference; AUDIT_METHODOLOGY § 4.27 + § 4.31 bank the methodology refinements.

**Source 5 (Equibase charts) operational expectation (per Phase A handoff § 5.1 open thread).** `download_charts.py` failing daily with exit=1, `new_pdfs=0` across captured days (May 9 / 10 / 11). SNS alerts may be firing daily to `arn:aws:sns:us-east-1:584812014683:equine-equalizer-alerts` until Tony disposition lands. Non-blocking for Source 4 workouts step (Source 4 producer `run_daily_refresh.sh` continues to write workout-load S3 objects; chart-failure does not propagate to Source 4 outputs). In-bible chart parser entry-point per § 4.1.3 (invoked via `parse_charts` admin action on `equine-ingestion`).

### 4.5 AWS API validation discipline (D6 v1-patched-e patch per Phase A handoff § 2.3)

**Substrate fact.** AWS Lambda's `PutFunctionEventInvokeConfig` API validates the target role's `sqs:SendMessage` permission on the destination DLQ ARN at API time, BEFORE event-invoke-config takes effect. If the role lacks the permission, the API call fails with `InvalidParameterValueException: The function execution role does not have permissions to call SendMessage on <DLQ ARN>`.

**Operational discipline.** For DLQ wiring across mixed greenfield/brownfield roles, IAM grant must precede event-invoke-config, sequentially — never parallel/chained. The grant-precedes-config requirement is API-validation-time, not eventual-consistency-time; brief IAM-propagation lag may add latency to grant-applied state but does not alter the precedence requirement.

**Dispatch-text discipline.** QB dispatch text must specify step ordering per API dependency requirements, not per logical-deliverable order:
...
**Banked instances across Phase A operational cycle.**

| Phase A dispatch | Lambdas wired | Step ordering executed | Outcome |
|---|---|---|---|
| Phase A-prime (2026-05-11) | 2 (`equine-ingestion` + `equine-results`) | IAM grant precedes config (CC discretion) | SUCCESS — dispatch-text inversion masked by CC ordering |
| A.5 (2026-05-12) | 3 (WR / PL / LS inference Lambdas) | IAM grant precedes config (CC discretion) | SUCCESS — dispatch-text inversion masked by CC ordering |
| A.5-ext (2026-05-12) | 1 (`equine-nyra-workouts`) | Literal dispatch order: event-invoke-config first | FAILED on first attempt with `InvalidParameterValueException`; recovered by reapplying after IAM policy succeeded; passed on first retry, no IAM-propagation lag observed |

The A.5-ext failure surfaced the dependency requirement; A.5-ext closure CC report banked the methodology refinement; AUDIT_METHODOLOGY § 4.30 catalogues the case study lesson.

**Future scope.** Any future Lambda DLQ wiring dispatch must use correct API-dependency-order in dispatch text. Generalization: any AWS API that validates IAM permissions on a target resource at API-call time (e.g., S3 `PutBucketNotification` destination-validation, EventBridge `PutTargets` ARN-permission-validation, KMS key-policy attachment) requires IAM-grant-precedes-config sequencing in dispatch text.

**Cross-reference.** AUDIT_METHODOLOGY § 4.30 (A.5-ext API step-ordering case study lesson) + AUDIT_METHODOLOGY § 4.31 (producer-attribution methodology refinement banking the broader pattern of dispatch-text inheritance vs substrate verification).

### 4.6 predict_race internal filter + A.5.3 fix substrate (D6 v1-patched-e patch per Phase A handoff § 2.5)

**Substrate location.** Two-layer filter substrate:

- **Outer (catch-all exception filter):** `backend/services/feature_engineering_service.py:94-116` — `build_feature_matrix` method's per-entry try/except wrapping `_build_entry_features` calls; exceptions during entry-feature build cause the entry to be silently dropped from the feature matrix.
- **Inner (per-helper NaN guard, post-A.5.3 fix):** `model/shared/gonzo_features.py:558` — `compute_gonzo_class_features` filter expression extended with `not pd.isna(pp_finishes[i])` guard.

**Pre-A.5.3 defect.** `compute_gonzo_class_features` at `gonzo_features.py:561` and `:566` performed `int(f)` on `at_or_above_finishes` list elements without NaN-handling; `ValueError: cannot convert float NaN to integer` was raised. The ValueError was caught by the outer try/except at `feature_engineering_service.py:110`; entry was silently dropped from the feature matrix without log surface above DEBUG. Empirical impact: 9 horses per day silently dropped from inference output (A.5.2 surfacing via 'Tapit Shoes' discriminator analysis across May 2 / 3 / 7 / 8 / 10 dates).

**Filter classification.** F-side-effect (exception-handler-based, not field-driven, heterogeneous failure modes). Distinct from the SQL-side race-eligibility filter at `backend/services/race_repository.py:66-94` (F-race-type + F-claim-price, intentional architectural filter; see `architecture_overview:4.4` D6 patch) and the in-memory field-size filter at WR/PL inference service iterations (`if len(race.entries) < 4: continue`). The exception-handler filter is non-architectural; it captures defects in compute helpers as silent drops.

**A.5.3 surgical fix (commit `e1d6d4a`; CDK deploy 122.41s; 7 Lambda asset hashes + 1 ECS TaskDef replace).**

- **File:** `model/shared/gonzo_features.py:558` filter expression.
- **Before:** `if t >= today_tier and pp_finishes[i] is not None`
- **After:** `if t >= today_tier and pp_finishes[i] is not None and not pd.isna(pp_finishes[i])`
- **Diff scope:** 1 file changed, 13 insertions (filter line + 12 lines comment block documenting fix rationale + Phase B substrate review flag), 2 deletions.
- **Verification:** 9 affected horses recovered (LatetotheGame, Toga d'Oro, Anmer Hall, On the Hill, Vancougar, MakerandSons, City Blocks, Tap Me a Song, Tapit Shoes); 7 predictions recovered across 5 dates (May 2 / 3 / 7 / 8 / 10); per-date deltas converged to 0. May 1 -2 delta persists as D2-γ scratched-after-prediction surplus (architectural; see § 4.7).

**Upstream gap flagged (Phase B candidate).** `_load_raw_pps` SQL filter at `gonzo_features.py:499-501` is `WHERE finish_position IS NOT NULL AND finish_position < 90` per docstring. Empirical evidence suggests pandas coercion of SQL-NULL or empty-string sentinel to NaN during `pd.read_sql_query` despite SQL-side filter; defense-in-depth at helper-level (the A.5.3 fix) is operationally correct, but the upstream pandas NaN-coercion mechanism is uncharacterized at v1-patched-e lock. Phase B substrate review candidate per `ml_layer_architecture_bible:8` (D6 patch).

**PREDICT_RACE_TOLERANCE=5 (interim defense-in-depth constant).**

- **Location:** `backend/lambdas/entries_tracks_publisher/handler.py` module constant; consumed by the `equine-entries-qualifying-tracks-missing` composite alarm's Expected metric SQL (per § 4.3 composite-alarm-pattern banking).
- **Rationale:** Max observed positive delta (+3 on May 3) + 2 safety buffer.
- **A.5.4 reduction candidate:** After 2–4 weeks observation post-A.5.3, reducible to 0 or 1 if no additional inference-time entry-drop modes surface.
...
**Train-test skew Phase B flag (per Phase A handoff § 2.14).** `model/shared/gonzo_features.py` is shared between training pipeline (ECS Fargate task families per `architecture_overview:3.2`) and inference Lambdas. A.5.3 fix scope is inference-only per F3 ratification; training-side semantics of the same NaN-guard are not substrate-verified at v1-patched-e lock. Phase B substrate review required per `ml_layer_architecture_bible:8.1` (D6 patch): (a) whether training pipeline exercises `compute_gonzo_class_features` at all; (b) data shape at training time; (c) whether NaN finish_positions are present in training inputs OR pre-filtered upstream; (d) train-test skew characterization; (e) re-train decision.

**Cross-references.** A.5.1 (predict_race filter trace + Expected SQL refinement applied PREDICT_RACE_TOLERANCE=5), A.5.2 (build_entry_features trace + May 1 forensic), A.5.3 (gonzo_features int(NaN) surgical fix) dispatch reports per Phase A handoff § 1.5. AUDIT_METHODOLOGY § 4.30 + § 4.31 (methodology refinements applied at A.5.3 checkpoint #11).

### 4.7 Scratched-after-prediction surplus pattern (D6 v1-patched-e patch per Phase A handoff § 2.15)

**Substrate.** May 1 CD R13 had 17 entries total; 2 scratched ('Bella Ballerina' pgm 12, 'My Miss Mo' pgm 6); 15 active at race-time; 17 WR-general predictions persisted in `wr_predictions` (one per entry including the 2 scratched). Discovered at A.5.2 forensic.

**Classification.** D2-γ variant — scratched-after-prediction surplus. Distinct from D2-α (entry-level inference drop; A.5.3 surface) and D2-β (race-level eligibility filter; § 4.4 + `architecture_overview:4.4`).

**Architectural correctness.** Predictions are history-preserving. Scratch event updates `entries.is_scratched=TRUE` but does NOT cascade-delete `wr_predictions` rows. Predictions persist for downstream consumers:

- `equine-results` matcher Lambda (Appendix B per `architecture_overview:3.12` D6 patch) backfilling `actual_finish` against arriving results.
- Retrospective analysis paths (Phase B model evaluation cycle).
- Audit trail of inference-time decisions.

**Expected vs Actual asymmetry.** The `equine-{wr,pl,ls}-predictions-deficit` composite alarms (per § 4.3 + `architecture_overview:3.10` extension):

- m1 (Expected) = forward-looking active entries (post-scratch eligibility — SQL mirrors A.6.c race-eligibility filter + applies PREDICT_RACE_TOLERANCE=5 post-fetch).
- m2 (Actual) = history-preserving prediction count.
- Math expression `IF(m1 > 0, m1 - m2, 0) > 0` correctly handles negative delta (Actual > Expected when scratched-after-prediction surplus): m1 - m2 < 0 → IF clamps to 0 → alarm stays OK.

**Disposition.** D6 documentation only; architecturally correct; no fix needed. Phase A handoff § 1.6 + § 5 do not bank a disposition recommendation; runbook scope at `docs/operations/DAILY_INGESTION_RUNBOOK.md § 2` documents the architecturally-correct asymmetry-handling.

**Cross-reference.** A.5.2 sub-finding 2 (May 1 scratched-after-prediction surplus) per Phase A handoff § 2.15. Inference Lambda DLQ + predictions-deficit alarm pattern at `ml_layer_architecture_bible:4.3` D6 patch.
...
## End of Data Pipeline Bible v1-patched-e (LOCKED — Phase 1 deliverable 3 of 7; locked 2026-05-12 via Phase A D6 bundled bible patches dispatch under Tier 2 ceremony cap; supersedes v1-patched-d 2026-05-11). 5 D6 patches applied: DP-1 NEW § 4.4 + DP-2 § 4.2.4 refresh + DP-3 NEW § 4.5 + DP-4 NEW § 4.6 + DP-6 NEW § 4.7. R14.2 Option A § 4.2.4 deferred-disposition reservation CLOSED. UC § 7.2 step 4 per-bible patch-CC convention overridden by Phase A entry directive ceremony cap; override disclosure per revision-history v1-patched-e entry above. Cross-bible cross-reference freeze NOT re-engaged for D6 (Tier 2 ceremony cap pattern). v1-patched-d footer historical content preserved below for substrate-evolution audit trail per banked Lesson § 4.17 (locked bibles preserve drafting-time historical context).

### v1-patched-d footer (historical retention per Lesson § 4.17)
