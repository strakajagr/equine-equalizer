...
def main():
    ...
    for variant_name, feature_names in variants:
        ...
        metrics, booster, preds_eval = train_variant(
            X_train, y_train, X_eval, y_eval, feature_names, variant_name
        )
        variant_preds[variant_name] = (booster, preds_eval)
    ...
    # 7b. Save BEST variant's per-row eval predictions for §4 P3 analysis.
    best_variant = max(results['variants'].items(), key=lambda kv: kv[1]['auc_eval'])[0]
    logger.info(f"Best variant by AUC: {best_variant}")
    best_booster, best_preds = variant_preds[best_variant]
    results['best_variant'] = best_variant

    eval_meta = full_df.loc[eval_mask, ['pp_id', 'race_id', 'race_date']].reset_index(drop=True)
    eval_meta['pred_win_prob'] = best_preds
    pp_to_horse = labels_df.set_index('pp_id')['horse_id']
    eval_meta['horse_id'] = pp_to_horse.loc[eval_meta['pp_id']].values
    # race_id here is the race_key = "track_YYYYMMDD_racenum"; parse out parts
    rk = eval_meta['race_id'].astype(str)
    eval_meta['track_code'] = rk.str.split('_').str[0]
    eval_meta['race_number'] = rk.str.split('_').str[-1].astype(int)
    preds_csv = '/tmp/gate3_predictions.csv'
    eval_meta[['race_id', 'horse_id', 'pred_win_prob',
               'race_date', 'race_number', 'track_code']].to_csv(preds_csv, index=False)
    logger.info(f"V1 predictions: {preds_csv} ({len(eval_meta):,} rows)")
    try:
        s3.upload_file(preds_csv, S3_BUCKET, f'{S3_PREFIX}/gate3_predictions_{ts}.csv')
        logger.info(f"Uploaded predictions to s3://{S3_BUCKET}/{S3_PREFIX}/gate3_predictions_{ts}.csv")
    except Exception as e:
        logger.warning(f"S3 upload of predictions failed: {e}")

    booster_path = '/tmp/gate3_best_booster.json'
    best_booster.save_model(booster_path)
    try:
        s3.upload_file(booster_path, S3_BUCKET, f'{S3_PREFIX}/gate3_best_booster_{ts}.json')
        logger.info(f"Uploaded booster to s3://{S3_BUCKET}/{S3_PREFIX}/gate3_best_booster_{ts}.json")
    except Exception as e:
        logger.warning(f"S3 upload of booster failed: {e}")
